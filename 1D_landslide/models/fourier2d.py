...
class FNN2d(nn.Module):
    ...
    def forward(self, x):
        ...
        length = len(self.ws)
        batchsize = x.shape[0]
        nx, nt = x.shape[1], x.shape[2] # original shape
        ...
        for i, (speconv, w) in enumerate(zip(self.sp_convs, self.ws)):
            x1 = speconv(x)
            x2 = w(x.view(batchsize, self.layers[i], -1)).view(batchsize, self.layers[i+1], size_x, size_t)
            x = x1 + x2
            if i != length - 1:
                x = self.activation(x)
        x = x.permute(0, 2, 3, 1)
        ...
        x = self.activation(x)
        x = self.fc2(x)
        x = x.reshape(batchsize, size_x, size_t, self.out_dim)
        x = x[..., :nx, :nt, :]
        return x
