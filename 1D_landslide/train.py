...
class DataLoader(Dataset):
    def __init__(self, datapath, h5file,gridx,gridt):
        self.datapath = datapath
        self.h5file = h5file
        self.x_list = []
        self.gridx=gridx
        self.gridt=gridt
        self.Nx=self.gridx.shape[1]
        self.Nt=self.gridx.shape[2]
        #self.y_list = []
        with open(self.datapath, 'r') as f_txt:
            for line in f_txt.readlines():
                xpath  = line.split(' ')[0]
                self.x_list.append(xpath)
                #self.y_list.append(ypath)
    ...
    def __getitem__(self, idx):
        ...
        data=torch.cat([self.gridx,self.gridt,data], dim=-1)
        self.data=data.squeeze(0)#(nx,nt,nin)
        self.data[:,:,4]=(self.data[:,:,4]-torch.min(self.data[:,0,4]))/1400#z_max
        self.data[:,:,2]=self.data[:,:,2]/1000 #c value kpa
        h0=self.data[:, 0, 5].reshape(1,self.Nx)/141#h_max
        h0=h0.unsqueeze(2).unsqueeze(3).repeat([1,1 , self.Nt, 1]).squeeze(0)
    ...
    def search_z(self,idx):
        ...
        xname = self.x_list[idx]
        data = torch.tensor(np.array(f_h5[xname])).float()
        self.data=data.squeeze(0)#(nx,nt,nin[c,fai,z,h,u])
        z=self.data[:,:,2]
        f_h5.close()
        return z
