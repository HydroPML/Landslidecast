...
class DataLoader1D(object):
    def __init__(self, data, nx=128, nt=100, sub=1, sub_t=1, nin=1, nout=2,new=True,tend=1):
#         dataloader = MatReader(datapath)
        self.sub = sub
        self.sub_t = sub_t
        self.nin = nin
        ...
        if new:
            self.T += 1
        data = data[:, 0:self.T:sub_t, 0:self.s:sub]
        self.data = data.permute(0, 2,1,3)

    def make_loader(self, n_sample, batch_size, start=0, train=True,xmin=0,xmax=1):
        a_data = self.data[start:start + n_sample, :, 0, :self.nin].reshape(n_sample, self.s,  self.nin)
        u_data = self.data[start:start + n_sample].reshape(n_sample, self.s, self.T, self.nout)
        #Xs = self.x_data[start:start + n_sample]
        #ys = self.y_data[start:start + n_sample]

        if self.new:
            gridx = torch.tensor(np.linspace(xmin, xmax, self.s + 1)[:-1], dtype=torch.float)
            gridt = torch.tensor(np.linspace(0, self.tend, self.T), dtype=torch.float)
        else:
            gridx = torch.tensor(np.linspace(xmin, xmax, self.s), dtype=torch.float)
            gridt = torch.tensor(np.linspace(0, self.tend, self.T + 1)[1:], dtype=torch.float)
        gridx = gridx.reshape(1, self.s,1,1)
        gridt = gridt.reshape(1,1, self.T, 1)

        a_data = a_data.reshape(n_sample, self.s,1,self.nin).repeat([1, 1,self.T, 1])
        a_data = torch.cat([gridx.repeat([n_sample,1, self.T, self.nin]), gridt.repeat([n_sample, self.s,1,self.nin]),a_data], dim=-1)
        dataset = torch.utils.data.TensorDataset(a_data, u_data)
        if train:
            loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True)
        else:
...
class DataLoader2D_swe(object):
    def __init__(self, data, nx=128,ny=128, nt=100, sub=1, sub_t=1, tend=1.0, nin=1, nout=3):
        self.sub = sub
        self.sub_t = sub_t
        self.tend = tend
        ...
        self.T = nt // sub_t
        self.T += 1
        data = data[:, 0:self.T:sub_t, 0:self.nx:sub, 0:self.ny:sub]
        self.data = data.permute(0, 2, 3, 1, 4)
        
    def make_loader(self, n_sample, batch_size, start=0, train=True,xmin=0,xmax=1,ymin=0,ymax=1):
        a_data = self.data[start:start + n_sample, :, :, 0, :self.nin].reshape(n_sample, self.SX, self.SY, self.nin)
        u_data = self.data[start:start + n_sample].reshape(n_sample, self.SX, self.SY, self.T, self.nout)
        gridx, gridy, gridt = get_grid3d(self.SX, self.SY,self.T, time_scale=self.tend,xmin=xmin,xmax=xmax,ymin=ymin,ymax=ymax)
        a_data = a_data.reshape(n_sample, self.SX, self.SY, 1, self.nin).repeat([1, 1, 1, self.T, 1])
        a_data = torch.cat((gridx.repeat([n_sample, 1, 1, 1, 1]),
                            gridy.repeat([n_sample, 1, 1, 1, 1]),
                            gridt.repeat([n_sample, 1, 1, 1, 1]),
                            a_data), dim=-1)
