...
def get_grid3d(SX,SY, T, time_scale=1.0,xmin=0,xmax=1,ymin=0,ymax=1, device='cpu'):
    gridx = torch.tensor(np.linspace(xmin, xmax, SX + 1)[:-1], dtype=torch.float, device=device)
    gridx = gridx.reshape(1, SX, 1, 1, 1).repeat([1, 1, SY, T, 1])
    gridy = torch.tensor(np.linspace(ymin, ymax, SY + 1)[:-1], dtype=torch.float, device=device)
    gridy = gridy.reshape(1, 1, SY, 1, 1).repeat([1, SX, 1, T, 1])
    gridt = torch.tensor(np.linspace(0, 1 * time_scale, T), dtype=torch.float, device=device)
    gridt = gridt.reshape(1, 1, 1, T, 1).repeat([1, SX, SY, 1, 1])
